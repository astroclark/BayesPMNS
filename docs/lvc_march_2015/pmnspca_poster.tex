...
\subfloat[Phase Spectra]{%
  \includegraphics[width=0.45\linewidth]{catalogue_phase_overlay.pdf}%
  \label{fig:}%
}
\caption{Magnitude (left) and phase (right) spectra of our waveform catalogue
    $\matr{X}$.  \emph{Top row}: spectra of the original waveforms.  Note the
    wide range in peak locations.  \emph{Bottom row}: spectra following the
    frequency shifting procedure.  Waveform frequencies have been scaled such
that the peaks align at 1\,kHz (red vertical line).\label{fig:catalogue}}
\end{minipage}
...
\subsection*{\centering Demonstration}
\begin{minipage}{\columnwidth}
\makeatletter
\newcommand{\@captype}{figure}
\makeatother
...
\caption{Example: reconstrucing the Shen 1.35+1.35 ${\rm M}_{\odot}$ waveform
